...
for (const file of htmlFiles) {
  ...
  // Content integrity: no mojibake or non-Latin script in English copy
  ...
  if (/\uFFFD|Ã.|â€/.test(textOnly)) fails.push(`${rel}: mojibake / encoding corruption detected`);
  if (/"aggregateRating"/.test(html)) fails.push(`${rel}: aggregateRating present — self-serving review markup (B-05)`);
  if (isHome && /"@type":"BreadcrumbList"/.test(html)) fails.push(`${rel}: homepage must not emit BreadcrumbList (H-08)`);
  const h1 = (html.match(/<h1[\s>]/g) || []).length;
  if (h1 !== 1) fails.push(`${rel}: expected exactly 1 <h1>, found ${h1}`);
  // F-01: no precise identifier may appear in an outbound wa.me link
  for (const link of (html.match(/https:\/\/wa\.me\/[^"'\s]+/g) || [])) {
    const t = decodeURIComponent(link).split("?text=")[1] || "";
    if (/#\d{1,3}-\d{1,4}/.test(t)) fails.push(`${rel}: unit number in a WhatsApp link (F-01)`);
    if (/\b\d{6}\b/.test(t)) fails.push(`${rel}: full postal code in a WhatsApp link (F-01)`);
    if (/\bBlock:\s*\d/i.test(t)) fails.push(`${rel}: block number in a WhatsApp link (F-01)`);
  }
  // A-06: never claim 24/7 unless genuinely offered
  if (/\b24\/7\b|24 hours a day|round-the-clock/i.test(html)) fails.push(`${rel}: unsupported 24/7 availability claim (A-06)`);
  ...
  while ((h = hrefRe.exec(html))) {
    ...
    if (!existsSync(r)) { if (t.startsWith("assets/") || t.startsWith("/assets/") || t.startsWith("../assets/")) continue; warns.push(`${rel}: link may be broken -> ${t}`); }
  }
}
const placeholders = [/example\.sg/i, /yourdomain/i, /TODO PAYNOW/i, /6500000000/];
...
/* Deployment guard: every non-404 page must use RELATIVE asset paths so the site
 * works at a domain root, in a sub-folder, and from a local preview server. */
for (const file of htmlFiles) {
  const rel = file.replace(root, "");
  if (rel.replace(/^\//, "") === "404.html") continue;   // 404 legitimately uses basePath
  const html = readFileSync(file, "utf8");
