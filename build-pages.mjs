...
const routes = ["/"];
const WA_CTA = waHref("Hi, I'd like a quote for a letterbox lock. My block/unit is __ and postal district is __.");
...
for (const s of SERVICE_PAGES) {
  ...
  <section class="section"><div class="wrap narrow prose">
        ${sectionsHtml}
        <p class="mt-12"><a class="btn order btn-lg" href="../index.html#order">🔒 Order now</a>
        <a class="btn green" data-wa href="${WA_CTA}" rel="noopener noreferrer">Get a WhatsApp quote</a></p>
    </div></section>`;
...
const legalBody = {
  "privacy-policy": `
    ...
    <p>This website does not store your personal data on its own servers. The order form runs entirely in your browser. When you press “Continue on WhatsApp”, the form builds a prefilled message and passes it to WhatsApp through the link (a URL). That message contains your product choice, quantity, mailbox condition, block, unit and 2-digit postal district. It does not contain your name or phone number — you type those inside the WhatsApp chat.</p>
