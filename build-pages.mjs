 ...
 * 404, robots and sitemap. Breadcrumbs only on genuine child pages (H-08).
 * ========================================================================== */
...
import { esc, attr, head, header, footer, scripts, runtimeData, waHref, telHref,
         comparisonTable, specValue, money, WA_GENERIC, WA_URGENT } from "./src/render.mjs";
...
const cta = `        <p class="mt-12"><a class="btn order btn-lg" data-ev="quote-nav" href="../index.html#quote">Get a Written Quote</a>
        <a class="btn green" data-wa data-ev="whatsapp" href="${WA}" rel="noopener noreferrer">Send photos on WhatsApp</a></p>`;

/* ---------- /products/ : full catalogue + specs + compare (M04/M05/M18) ---- */
{
  const slug = "products";
  const h1 = "All 9 Letterbox Locks — Prices, Specifications & Which to Choose";
  ...
  const cards = PRODUCTS.map((p, i) => {
    ...
    return `      <article class="pdp" id="${attr(p.id)}" data-categories="${attr(p.categories.join(","))}">
        ...
        <div class="pdp-body">
          <p class="code">${esc(p.id)}${p.popular ? ` · <span class="tagpop-inline">Most popular</span>` : ""}</p>
          <h2>${esc(p.name)}</h2>
          <p class="pdp-price"><strong>${money(p.price)}</strong> <small>supply + standard installation</small></p>
          <p>${esc(p.desc)}</p>
          ...
          <details class="specs"><summary>Full specifications</summary>
            <dl class="spec-list">
              ...
              ${s.operatingTempC !== undefined ? specRow("Operating temperature", specValue(s.operatingTempC)) : ""}
              ${s.ingressRating !== undefined ? specRow("Ingress / weather rating", specValue(s.ingressRating)) : ""}
              ${specRow("Workmanship warranty", specValue(p.warrantyMonths ? `${p.warrantyMonths} months` : null))}
            </dl>
            <p class="spec-note">Figures marked “pending supplier confirmation” are verified against the manufacturer datasheet and stated in your written quote before installation. We do not publish specifications we cannot evidence.</p>
          </details>
          <div class="add-row">
  ...
  const itemListLd = JSON.stringify({
    "@context": "https://schema.org", "@type": "ItemList", name: "Letterbox lock models",
    itemListElement: PRODUCTS.map((p, i) => ({ "@type": "ListItem", position: i + 1, name: `${p.id} ${p.name}`, url: `${B}/products/#${p.id}` })),
  });
  const ld = `${crumbLd(slug, "All letterbox locks")}</script><script type="application/ld+json">${itemListLd}`;

  const body = `  <section class="page-hero"><div class="wrap">
${crumbs(h1)}
      <h1>${esc(h1)}</h1>
      <p>Every model we fit, with honest advantages and limitations. Prices include supply and standard installation on a compatible, accessible mailbox.</p>
    </div></section>
...
${cta}
  ...
  writePage(slug, shell({ slug, title: "All 9 Letterbox Locks Singapore | Prices & Specifications", description: "Compare all nine letterbox lock models we fit in Singapore — keyed, combination and electronic PIN. Prices from S$60, with honest pros and limits.", ld, body }));
}

/* ---------- /compatibility-guide/ (M01/M02/M17) --------------------------- */
{
  const slug = "compatibility-guide";
  const h1 = "Will a New Lock Fit My Letterbox? Compatibility Guide";
  const steps = COMPATIBILITY.photos.map((p) => `        <li><strong>${esc(p.h)}</strong><span>${esc(p.p)}</span></li>`).join("\n");
  const nofit = NO_FIT_POLICY.map((n) => `<h3>${esc(n.h)}</h3><p>${esc(n.p)}</p>`).join("\n      ");
  const limits = SERVICE_LIMITS.map((l) => `<li>${esc(l)}</li>`).join("");
  const body = `  <section class="page-hero"><div class="wrap">
${crumbs(h1)}
      <h1>${esc(h1)}</h1>
      <p>Most standard HDB and condominium letterbox doors take one of our nine models. This is how we check before you commit to anything.</p>
    </div></section>
  <section class="section"><div class="wrap narrow prose">
      <h2>The three photos we need</h2>
...
${steps}
      ...
      <p>This is why a photo of the door edge matters as much as the front. A lock that looks identical online can still be 4&nbsp;mm too short for your door.</p>
      <h2>If the lock still does not fit</h2>
      ${nofit}
      <h2>Mailboxes we may not be able to service</h2>
      <ul>${limits}</ul>
      <h2>Measuring it yourself</h2>
      <p>You do not need to measure anything — photos are enough for us. If you prefer to, the useful figure is the door panel thickness at the edge, in millimetres, and the diameter of the existing lock hole. Send those alongside the photos and we can usually confirm a model immediately.</p>
${cta}
    </div></section>`;
  writePage(slug, shell({ slug, title: "Will It Fit? Letterbox Lock Compatibility Guide Singapore", description: "How to check a replacement letterbox lock will fit your HDB or condo mailbox — the three photos we need, what decides compatibility, and our no-fit policy.", ld: crumbLd(slug, h1), body }));
}

/* ---------- /repair-or-replace/ (M06) ------------------------------------- */
{
...
${rows}
      ...
      <p>If a key has been forced and the cylinder is damaged, if the code on a combination lock has been forgotten, or if the mechanism no longer engages the cam reliably, replacement is the right call. In those cases we open the mailbox first so your mail is accessible the same visit, then fit the model you choose.</p>
...
for (const s of SERVICE_PAGES) {
  ...
  const ld = `${crumbLd(s.slug, s.h1)}</script><script type="application/ld+json">${serviceLd}`;
...
${crumbs(s.h1)}
  ...
  <section class="section"><div class="wrap narrow prose">${urgentBlock}
        ${sections}
...
/* ---------- /about/ (B-06 + M07 authorisation procedure) ------------------- */
{
  const slug = "about";
...
${crumbs(h1)}
  ...
  <section class="section"><div class="wrap narrow prose">
      ...
      <p>We work on letterbox and mailbox locks only: HDB letterboxes, condominium mail compartments, landed-home mailboxes and weather-exposed outdoor letterboxes. We replace worn or jammed locks, open mailboxes when the only key is lost, and upgrade residents to keyless combination or electronic PIN locks. We do not take main-door, gate, vehicle, padlock or safe work, so every visit is made by someone who works on letterboxes every day and carries the right parts.</p>
      <h2>How a job runs</h2>
      <p>You send three photos on WhatsApp — the mailbox front, the inside latch and the door edge. We assess compatibility, recommend a model and issue a written price that itemises the lock, the labour and any opening fee. On site the lock is fitted, aligned, tested and demonstrated to you. Payment is made afterwards by PayNow, cash or bank transfer. There is nothing to pay online, no deposit, and nothing is charged for an assessment.</p>
      ...
      <p>Standard supply and installation starts from S$60 and lost-key opening from S$25 per unit, with the full nine-model range and specifications published on our <a href="../products/index.html">products page</a>. Group and MCST orders receive tiered discounts, and orders of twenty units or more are always priced by written quote after a survey rather than an automatic total.</p>
...
${cta}
  ...
  writePage(slug, shell({ slug, title: `About Us | ${SITE.businessName}`, description: "Singapore's dedicated letterbox and mailbox lock specialist. How we work, our authorisation checks before opening any mailbox, service hours, limits and warranty.", ld, body }));
}
...
const legalBody = {
  ...
  "terms": `
    ...
    <p>Any figure calculated on this website is an <strong>estimate only</strong> and is not a confirmed price or an offer. A binding price is issued in writing after we review photographs of your mailbox. Orders of 20 units or more are priced by written quote following a survey. Where a browser estimate and our written quotation differ, the written quotation prevails.</p>
    <h2>What the starting price covers</h2>
    <p>The published starting price assumes a compatible, accessible mailbox and covers supply of the lock, removal of the old mechanism, fitting, alignment and demonstration. Opening a locked mailbox, repairs to damaged or rusted doors, non-standard enclosures and any urgent surcharge are charged separately and quoted in advance.</p>
    <h2>Compatibility and our no-fit policy</h2>
    <p>Where we have confirmed a model from your photographs and it does not fit, we will fit a suitable alternative at the confirmed price where one exists, or leave the mailbox as found at no charge, including no attendance fee. Where a condition was not reasonably visible in the photographs, we will stop, explain, and provide a revised written price which you are free to decline at no charge.</p>
    ...
    <h2>Payment &amp; GST</h2>
    <p>We accept cash, PayNow and bank transfer. Payment is due on completion unless otherwise agreed in writing. ${SITE.paymentInfo.gstRegistered ? `All prices are inclusive of ${Math.round(SITE.paymentInfo.gstRate * 100)}% GST.` : "We are not GST-registered, so no GST is added to your bill."}</p>
    <h2>Liability</h2>
  ...
  "warranty": `
    ...
    <p>Wear from misuse, forced entry, vandalism, water ingress on mailboxes unsuitable for outdoor exposure, or a user-set code that has been forgotten. Consumables such as batteries are excluded. Damage arising from the mailbox door itself — rust, distortion or a previous non-standard repair — is not covered by the lock warranty.</p>
    <h2>How to claim</h2>
  ...
  "cancellation": `
    ...
    <p>Where we confirmed a model from your photographs and it cannot be fitted, no charge applies — including no attendance fee. See our compatibility and no-fit policy for the detail.</p>
    <h2>Refunds</h2>
...
/* ---------- 404 (K-06) ----------------------------------------------------
 * A 404 can be served at any URL depth, so relative asset paths cannot work.
 * We therefore use SITE.basePath ("" at domain root, "/repo" on a project page). */
{
  ...
  <section class="section"><div class="wrap narrow prose">
      ...
      <ul class="related">
        <li><a href="${BP}/products/index.html">All 9 letterbox locks, prices and specifications</a></li>
        <li><a href="${BP}/compatibility-guide/index.html">Will a new lock fit my letterbox?</a></li>
        <li><a href="${BP}/repair-or-replace/index.html">Repair or replace your letterbox lock</a></li>
...
/* ---------- robots + sitemap (H-04/H-05) ---------------------------------- */
writeFileSync(new URL("./robots.txt", root),
  `User-agent: *\nAllow: /\n\n# Public assets must stay crawlable so Google can render pages\nAllow: /assets/\n\nSitemap: ${B}/sitemap.xml\n`);
const urls = routes.map((r) => `  <url><loc>${B}${r}</loc><lastmod>${today}</lastmod><changefreq>${r === "/" ? "weekly" : "monthly"}</changefreq><priority>${r === "/" ? "1.0" : r === "/products/" ? "0.9" : "0.7"}</priority></url>`).join("\n");
