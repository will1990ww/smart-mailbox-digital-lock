/* build-pages.mjs — service pages, legal pages, robots.txt, sitemap.xml. Run after build.mjs. */
import { writeFileSync, mkdirSync } from "node:fs";
import { SITE, SERVICE_PAGES, LEGAL_PAGES } from "./src/data.mjs";
import { esc, head, header, footer, scripts, runtimeData, waHref } from "./src/render.mjs";

const B = SITE.baseUrl;
const root = new URL("./", import.meta.url);
const routes = ["/"];
const WA_CTA = waHref("Hi, I'd like a quote for a letterbox lock. My block/unit is __ and postal code is __.");
...
for (const s of SERVICE_PAGES) {
  ...
  <section class="section"><div class="wrap narrow prose">
        ${sectionsHtml}
        <p class="mt-12"><a class="btn order btn-lg" data-ev="order-nav" href="../index.html#order">🔒 Order now</a>
        <a class="btn green" data-wa data-ev="whatsapp" href="${WA_CTA}" rel="noopener noreferrer">Get a WhatsApp quote</a></p>
    </div></section>`;
...
const legalBody = {
  "privacy-policy": `
    <h2>Who we are</h2>
    <p>${esc(SITE.businessName)} provides letterbox and mailbox lock services across Singapore. You can reach us on WhatsApp for any privacy request.</p>
    <h2>What this website collects</h2>
    <p>This website does not store your personal data on its own servers. The order form runs entirely in your browser. When you press “Continue on WhatsApp”, the form builds a prefilled message and passes it to WhatsApp through the link (a URL). That message contains your product choice, quantity, mailbox condition, block, unit and postal code. It does not contain your name or phone number — you type those inside the WhatsApp chat.</p>
    <h2>Analytics</h2>
    <p>If enabled, we use a privacy-friendly, cookieless analytics measure that records only anonymous, aggregated events (such as a WhatsApp button click) on our own server. It does not use cookies, does not track you across other websites, and never stores your name, phone number or address.</p>
    <h2>Information you share in WhatsApp</h2>
    <p>Any appointment details you type in the WhatsApp chat (name, contact number, full address) are processed by WhatsApp under its own privacy terms and by us solely to provide the quotation and service you requested. We use it to schedule, perform and support the job, and to keep records required by law.</p>
    ...
    <p>Under Singapore's PDPA you may request access to, or correction of, the personal data we hold, and withdraw consent. Contact us on WhatsApp to make a request.</p>
    <h2>Third parties</h2>
...
const urls = routes.map((r) => `  <url><loc>${B}${r}</loc><lastmod>${today}</lastmod><changefreq>${r === "/" ? "weekly" : "monthly"}</changefreq><priority>${r === "/" ? "1.0" : "0.7"}</priority></url>`).join("\n");
