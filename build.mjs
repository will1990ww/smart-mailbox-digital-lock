/* =============================================================================
 * build.mjs — homepage. M19: short commercial summary that routes detail to
 * dedicated pages. No self-serving review schema, no homepage breadcrumb.
 * ========================================================================== */
import { writeFileSync } from "node:fs";
import { SITE, PRODUCTS, FAQS, AREAS, REVIEWS, COMPATIBILITY, NO_FIT_POLICY, GALLERY, COLOURS } from "./src/data.mjs";
import { esc, attr, head, header, footer, scripts, runtimeData, waHref, telHref, comparisonTable, money, WA_GENERIC, WA_URGENT, WA_GROUP } from "./src/render.mjs";

const B = SITE.baseUrl;
const root = new URL("./", import.meta.url);
const P = SITE.paymentInfo;
...
const gstLine = P.gstRegistered
  ? `All prices are inclusive of ${Math.round(P.gstRate * 100)}% GST.`
  : `We are not GST-registered, so no GST is added — the price we confirm in writing is the price you pay.`;

/* M19: only the three recommended models appear on the homepage. */
const FEATURED = ["P2", "P3", "P9"];
...
function card(p, i) {
  ...
  return `        <article class="prod">
      ...
      <div class="body">
        ...
        <p class="card-link"><a href="products/#${attr(p.id)}">Full specifications &amp; considerations →</a></p>
      </div></article>`;
}
...
function graphLd() {
  ...
  const business = {
    ...
    description: "Singapore's dedicated letterbox and mailbox lock locksmith. We replace, open and upgrade HDB, condo, landed and outdoor letterbox locks — keyed, battery-free combination and electronic PIN models from S$60. We do not service door, gate, car or safe locks.",
    slogan: SITE.slogan,
    ...
    hasOfferCatalog: { "@type": "OfferCatalog", name: "Letterbox lock products",
      itemListElement: PRODUCTS.map((p) => ({ "@type": "Offer", itemOffered: { "@type": "Product", name: p.name }, priceCurrency: "SGD", price: String(p.price) })) },
  };
  ...
  return JSON.stringify({ "@context": "https://schema.org", "@graph": [
    ...
    { "@type": "Service", "@id": `${B}/#service`, serviceType: "Letterbox and mailbox lock replacement, opening and installation",
...
const photoSteps = COMPATIBILITY.photos.map((p) => `        <li><strong>${esc(p.h)}</strong><span>${esc(p.p)}</span></li>`).join("\n");
...
const reviewsSection = HAS_REVIEWS ? `
  <section class="section" id="reviews">
    <div class="wrap">
      ...
      <p class="section-intro">Average rating ${AVG} out of 5 from ${VERIFIED.length} verified reviews.</p>
      ...
      ${SITE.reviewSourceUrl ? `<p class="review-src"><a href="${attr(SITE.reviewSourceUrl)}" rel="noopener noreferrer nofollow" target="_blank">Read and verify these reviews at our public profile →</a></p>` : ""}
    </div>
  </section>` : "";
...
<head>
${head({
  title: "Letterbox Lock Replacement & Locksmith Singapore | from S$60",
  description: "Singapore's dedicated letterbox lock locksmith. HDB, condo and outdoor mailbox lock replacement, lost-key opening and keyless upgrades from S$60.",
...
<main id="main">
  <section class="hero">
    <div class="wrap hero-grid">
      <div>
        ...
        <h1>Letterbox Lock Replacement &amp; Locksmith <span>in Singapore</span></h1>
        <p class="lead">Letterbox and mailbox locks are all we do — HDB, condo, landed and outdoor. Send three photos, we confirm the model and price in writing, then fit and test it. From S$60 installed.</p>
        <div class="hero-actions">
          <a class="btn order btn-lg" data-ev="quote-hero" href="#quote">Get a Written Quote</a>
          <a class="btn blue" href="products/index.html">Compare all 9 locks</a>
        </div>
        <div class="proof">
          <span><i>✓</i> From S$60 installed</span>
      ...
      <div class="visual">
        <div class="urgent-card">
          ...
          <p class="urgent-p">Send one photo of the mailbox door and your postal sector. We reply with current appointment availability and a written price before we travel.</p>
          ...
          <p class="urgent-note">We are open 09:00–21:00 daily and are not a 24-hour emergency service. ${esc(SITE.urgent.surchargeNote)}</p>
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="trustbar"><div class="wrap trustgrid">
    <div><b>Letterbox only</b><span>not a general locksmith</span></div>
    <div><b>S$60</b><span>installation from</span></div>
    <div><b>S$25</b><span>lost-key opening from</span></div>
    <div><b>Photo-first</b><span>fit confirmed before you commit</span></div>
  </div></section>
...
${photoSteps}
        ...
        <p class="mt-12"><a class="btn green" data-wa data-ev="whatsapp" href="${attr(waHref(WA_GENERIC))}" rel="noopener noreferrer">Send photos on WhatsApp</a>
...
${noFit}
  ...
  <section class="section" id="products">
    <div class="wrap">
      <span class="eyebrow">Recommended models</span>
      <div class="section-head">
        <div><h2>Three locks that suit most Singapore mailboxes</h2>
          <p class="section-intro">These cover the common cases. The full nine-model range, with specifications and honest limitations, is on the products page.</p></div>
        <a class="btn blue" href="products/index.html">See all 9 locks &amp; specifications</a>
      </div>
...
${comparisonTable("")}
      <p class="fine">Prices assume a compatible, accessible mailbox. Lost-key opening is charged per unit (from S$25). The final price is always confirmed in writing.</p>
    </div>
  </section>

  <section class="section white" id="pricing-scope">
    <div class="wrap two">
      <div>
      ...
      <div>
        ...
        <p class="fine">Anything outside the standard scope is quoted in writing before we begin. You are never charged for an assessment.</p>
      </div>
    </div>
  </section>
  ...
  <section class="section" id="how">
    <div class="wrap two">
      <div>
        ...
        <h2>Photos first. Written price. Then we fit it.</h2>
        <div class="feature-list">
          <div class="mini"><strong>1. Send three photos</strong><p>Mailbox front, inside latch and door edge.</p></div>
          <div class="mini"><strong>2. We confirm the model</strong><p>Compatibility checked, then a written price with every charge itemised.</p></div>
          <div class="mini"><strong>3. Book a slot</strong><p>Share your block, unit and preferred time inside the WhatsApp chat.</p></div>
          <div class="mini"><strong>4. Fit, test and pay</strong><p>Fitted, aligned and demonstrated. Pay by PayNow or cash afterwards.</p></div>
        </div>
      </div>
      <div class="steps"><ol>
...
${galleryCells}
  ...
  <section class="section group-sec" id="group">
    <div class="wrap"><div class="group-inner">
      ...
      <div class="group-card">
        ...
        <p class="muted-sm">Send your estate name, unit count and photos of one representative mailbox.</p>
        <ul>
  ...
  <section class="section white" id="payment">
    <div class="wrap payment-layout">
      <div>
        <span class="eyebrow">Payment &amp; GST</span>
        <h2>Pay after the job — PayNow, cash or transfer</h2>
        <p class="section-intro">There is nothing to pay online and no deposit. Once the lock is installed and tested, settle the written price however suits you.</p>
        <div class="pay-methods">${P.methods.map((m) => `<span class="pay-chip">${esc(m)}</span>`).join("")}</div>
        <div class="gst-note"><strong>GST:</strong> ${esc(gstLine)}</div>
      </div>
      <div class="paynow-card">
        <p class="paynow-head"><span class="paynow-logo">PayNow</span></p>
        <img class="paynow-qr" src="${attr(P.paynowQr)}" alt="PayNow QR code for ${attr(P.paynowName)}" width="240" height="240" loading="lazy" decoding="async">
        <p class="paynow-name">${esc(P.paynowName)}</p>
        <p class="paynow-id">PayNow: <strong>${esc(P.paynowId)}</strong></p>
        <p class="paynow-verify">✓ Before paying, check your banking app shows the name <strong>${esc(P.paynowName)}</strong>.</p>
      </div>
    </div>
  </section>
  ...
  <section class="section" id="quote">
    <div class="wrap order-layout">
      <div>
        ...
        <p class="section-intro">Choose one lock or build a list for multiple letterboxes. WhatsApp opens with your request ready to send; you add photos, your name, block and unit inside the chat.</p>
        <ul class="check-list">
          <li>✓ Itemised estimate with every charge shown</li>
          <li>✓ Bulk discount applied automatically</li>
          <li>✓ Compatibility confirmed from your photos first</li>
      ...
      <form class="form" id="quoteForm" novalidate>
        ...
        <div id="multiPane" hidden>
          <div class="ql-panel">
            ...
            <p class="ql-empty" id="listEmpty">No locks added yet — use <b>Add to quote</b> on any product.</p>
        ...
        <div class="form-grid">
          ...
          <label>Preferred date / time <span class="opt">(optional)</span><input id="qTiming" name="timing" maxlength="80" placeholder="e.g. Saturday afternoon"></label>
          <label>Block no. <span class="opt">(optional)</span><input id="qBlock" name="block" autocomplete="off" maxlength="6" placeholder="e.g. 123 or 123A">
            <span class="field-error" data-error-for="block"></span></label>
          <label>Unit no. <span class="opt">(optional)</span><input id="qUnit" name="unit" autocomplete="off" maxlength="12" placeholder="e.g. 12-345 or 1518">
...
console.log(`✓ index.html — ${featured.length} featured of ${PRODUCTS.length} products, ${VERIFIED.length} verified review(s), reviewSchema:${!!(SITE.reviewSchema?.emit && HAS_REVIEWS)}`);
