...
/* GST clarity line — honest for a non-registered small business, and correct
 * once you register (set paymentInfo.gstRegistered = true). */
const P = SITE.paymentInfo;
const gstLine = P.gstRegistered
  ? `All prices are inclusive of ${Math.round(P.gstRate * 100)}% GST.`
  : `We are not GST-registered, so there is no GST added to your bill — the price you are quoted is the price you pay.`;
...
function productCard(p) {
  ...
  const wa = attr(waHref(`Hi, I'd like a quote for the ${p.id} ${p.name} (from ${money(p.price)}). My block/unit is __ and postal code is __.`));
      ...
      <div class="body">
        ...
        <div class="add-row">
          <label class="add-colour"><span class="sr-only">Colour for ${esc(p.name)}</span>
            <select data-card-colour="${attr(p.id)}" aria-label="Colour for ${attr(p.name)}">${p.colours.map((c) => `<option value="${attr(c)}">${esc(COLOURS[c]?.label || c)}</option>`).join("")}</select></label>
          <button type="button" class="btn blue add-btn" data-add-product="${attr(p.id)}" data-ev="add_to_cart">Add to cart</button>
        </div>
        <div class="actions-2">
          <a class="btn ghost" href="${wa}" data-wa-product="${attr(p.id)}" data-ev="whatsapp-product" rel="noopener noreferrer">Quick WhatsApp quote</a>
        </div>
      </div></article>`;
}
function soonCard(u) {
  ...
  return `        <article class="soon-card" data-categories="smart">
      ...
      <ul>${feats}</ul>
      <a class="notify" href="${wa}" data-wa-product="${attr(u.id)}" data-ev="notify" rel="noopener noreferrer">Notify me when available</a>
    </article>`;
}
...
const paymentChips = P.methods.map((m) => `<span class="pay-chip">${esc(m)}</span>`).join("");
...
${head({
  ...
  description: "Singapore's dedicated letterbox lock locksmith — we only do letterbox & mailbox locks. HDB & condo letterbox lock replacement, lost-key unlocking and smart digital keyless installation. Every kind of letterbox lock, from S$50. Photo-first written quote on WhatsApp.",
...
<main id="main">
  <section class="hero">
    <div class="wrap hero-grid">
      <div>
        <span class="label">✓ SINGAPORE'S DEDICATED LETTERBOX LOCK LOCKSMITH</span>
        ...
        <div class="hero-actions">
          <a class="btn order btn-lg" data-ev="order-hero" href="#order">🔒 Order now</a>
  ...
  <section class="section white">
    <div class="wrap">
      ...
      <p class="section-intro">As a specialist letterbox locksmith, we install and replace HDB and condominium letterbox locks (also spelt letter box locks) and mailbox locks across Singapore — whether your key is lost, your lock is jammed, or you want a more secure keyless upgrade. Choose from traditional key, battery-free combination code, and electronic smart digital PIN models. Every job starts with a photo and a clear written price. Standard supply and installation starts from S$50, lost-key unlocking from S$25, with nine models up to S$85.</p>
    </div>
  </section>
    ...
    <div class="wrap two">
      <div>
        ...
        <div class="feature-list">
          ...
          <div class="mini"><strong>3. Book a slot</strong><p>Share your block, unit, postal code and preferred time.</p></div>
  ...
  <section class="section group-sec" id="group">
    <div class="wrap"><div class="group-inner">
      ...
      <div class="group-card">
        ...
        <a class="btn green btn-block" data-wa-group data-ev="whatsapp-group" href="${attr(waHref("Hi, I'd like a GROUP/BULK quote for letterbox locks. Estate: ___ | Units: ___ | Postal code: __"))}" rel="noopener noreferrer">Request group quote on WhatsApp</a>
        <div class="mt-12"><a class="btn white btn-block" data-tel data-ev="call" href="${attr(telHref())}">Call us</a></div>
      </div>
    </div></div>
  </section>
  ...
  <section class="section" id="payment">
    <div class="wrap payment-layout">
      <div>
        ...
        <h2>Easy payment by PayNow, cash or bank transfer</h2>
        <p class="section-intro">Pay the way that suits you once the job is done and tested. Scan the PayNow QR below with your banking app, or pay by cash or bank transfer.</p>
        <div class="pay-methods">${paymentChips}</div>
        <div class="gst-note"><strong>GST:</strong> ${esc(gstLine)}</div>
        <p class="fine">A written quotation is always provided before any work begins. Payment is due on completion unless otherwise agreed in writing.</p>
      </div>
      <div class="paynow-card">
        <div class="paynow-head"><span class="paynow-logo" aria-hidden="true">PayNow</span></div>
        ...
        <p class="paynow-hint">Scan the QR, or key in the PayNow number above.</p>
        <p class="paynow-verify">✓ Before you pay, check your banking app shows the name <strong>${esc(P.paynowName)}</strong>.</p>
      </div>
    </div>
  </section>
  <section class="section" id="order">
    <div class="wrap order-layout">
      <div>
        ...
        <p class="section-intro">Add one or more locks to your cart above, or pick a single lock below. Then add your block, unit and mailbox condition so we can schedule the visit. WhatsApp opens with your full order ready to send — you just add your name and contact number in the chat to confirm. Payment is by PayNow or cash <strong>after</strong> the job is done and tested.</p>
        <ul class="check-list">
          <li>✓ Product, colour, quantity and estimate included</li>
          <li>✓ Block, unit and postal code for scheduling</li>
        ...
        <div class="cart-panel" id="cartPanel" hidden>
          <div class="cart-panel-head"><h3>Your cart</h3><button type="button" class="cart-clear" id="cartClear">Clear</button></div>
          <ul class="cart-lines" id="cartLines"></ul>
          <div class="cart-total" id="cartTotal"></div>
          <p class="cart-note">Estimate only — final price is confirmed in writing after a photo. You pay by PayNow or cash after the job.</p>
        </div>
        <div class="privacy-box">
          <strong>How your privacy is handled:</strong> pressing “Continue on WhatsApp” builds a prefilled message containing your product choice(s), quantity, mailbox condition, <strong>block, unit</strong> and postal code. Your <strong>name and phone number are never put in the link</strong> — you type those inside the encrypted WhatsApp chat. See our <a href="privacy-policy/">Privacy Policy</a>.
        </div>
      </div>
      <form class="form" id="orderForm" novalidate>
        <div class="form-grid">
          ...
          <label>Unit no.<input id="orderUnit" name="unit" inputmode="text" autocomplete="off" maxlength="12" required placeholder="e.g. 12-345 or 1518" aria-describedby="err-unit">
            <span class="field-error" id="err-unit" data-error-for="unit"></span></label>
          <label>Postal code<input id="orderDistrict" name="district" inputmode="numeric" pattern="[0-9]{2,6}" minlength="2" maxlength="6" required placeholder="e.g. 520123 (or just 52)" aria-describedby="err-district">
...
<a class="cart-fab" id="cartFab" href="#order" aria-label="View cart" hidden>🛒<span class="cart-fab-count" id="cartCount">0</span></a>
...
console.log(`✓ Wrote index.html — ${PRODUCTS.length} products, ${REVIEWS.length} reviews, GST-registered:${P.gstRegistered}, analytics:${SITE.analytics.enabled}.`);
