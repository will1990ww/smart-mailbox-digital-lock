...
export const SITE = {
  ...
  reviewSourceUrl: "https://carousell.app.link/ZKDnEplci6b",
  ...
  /* ---- Payment & GST -------------------------------------------------------
   * Most Singapore small businesses under the S$1M turnover threshold are NOT
   * GST-registered, so by default we state prices carry no GST. If/when you
   * register, set gstRegistered:true and gstRate (currently 9%). The PayNow QR
   * image lives at assets/img/paynow-qr.png — replace the placeholder with your
   * real QR before launch. */
  paymentInfo: {
    methods: ["Cash", "PayNow", "Bank transfer"],
    paynowQr: "assets/img/paynow-qr.png",         // TODO replace with real PayNow QR
    paynowName: "ZXXXX WXX",        // registered payee name to verify
    paynowId: "+65 8341 7888",                     // TODO your PayNow mobile or UEN (shown as text so customers can pay without scanning)
    gstRegistered: false,                          // TODO set true if you register for GST
    gstRate: 0.09,
  },

  /* ---- Analytics (privacy-friendly, cookieless, CSP-safe) ------------------
   * OFF by default. To enable, set enabled:true and point `endpoint` at a
   * FIRST-PARTY collector on your own domain (keeps strict CSP: connect-src
   * 'self'). It sends only an event name + a few non-PII properties via
   * navigator.sendBeacon. No cookies, no fingerprinting, no personal data. */
  analytics: {
    enabled: true,                                // TODO set true once endpoint is ready
    endpoint: "/api/collect",                      // first-party path on your domain
  },
...
export const FAQS = [
  ...
  { q:"Are you a locksmith for letterboxes and mailboxes?",
    a:"Yes — and that is all we do. We are a dedicated letterbox lock locksmith serving all of Singapore. Whether you searched for a letterbox locksmith, mailbox locksmith, letter box lock, digital letterbox lock, combination letterbox lock or mailbox lock replacement, you are in the right place. We handle every kind of letterbox and mailbox lock — HDB, condo, landed and outdoor — but we do not do door, gate, car or safe locks, so you always get a specialist, not a generalist." },
  { q:"Do you supply outdoor or weather-exposed letterbox locks?",
    a:"Yes. For landed homes and gate-side or outdoor mailboxes we recommend our zinc-alloy models, which resist weather better than standard steel. Send a photo and we will suggest the most durable option for your location." },
];
