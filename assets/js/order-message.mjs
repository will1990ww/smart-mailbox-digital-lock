...
/* Postal code: keep the digits the customer typed (2–6), no forced truncation. */
export function toDistrict(value) { const s = String(value || "").replace(/\D/g, ""); return s.slice(0, 6); }
/* Unit: tidy only — uppercase and trim. We do NOT force a leading "#" or a dash,
 * so a plain number like 1518 or 13533 passes through unchanged. */
...
export function buildProductEnquiry({ productId, productName, price }) {
  return `Hi, I'd like a quote for the ${productId} ${productName} (from S$${price}). My block/unit is __ and postal code is __.`;
}
