/* QUOTE MESSAGE — WhatsApp deep link. Only NON-PRECISE data enters the URL. */
...
export function buildQuoteMessage({ items = [], result, accessLabel = "", accessFeePerUnit = 0, postal = "", timing = "", urgent = false }) {
  ...
  lines.push("------------------------------");
  lines.push("I'll send a photo of the mailbox front, plus my name, block and unit, here in the chat.");
  let msg = lines.join("\n");
...
export function buildProductEnquiry({ productId, productName, price }) {
  return `Hi, I'd like a written quote for the ${productId} ${productName} (from S$${price}), supplied and installed. I'll send a photo of my mailbox.`;
}
